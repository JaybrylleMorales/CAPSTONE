<x-layouts::app :title="'Edit Learning Path'">

<div class="space-y-6">

    <div>
        <h1 class="text-2xl font-bold">Edit Learning Path</h1>
        <p class="text-gray-500">
            Update path details and assigned courses.
        </p>
    </div>

    <div class="rounded-xl border bg-white p-6 shadow-sm dark:bg-neutral-900 dark:border-neutral-700">

        <form action="{{ route('learning-paths.update', $learningPath) }}"
              method="POST"
              class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text"
                       name="name"
                       value="{{ old('name', $learningPath->name) }}"
            ...
            <div>
                ...
                <textarea name="description"
                          rows="4"
                          class="w-full rounded border p-2 bg-transparent">{{ old('description', $learningPath->description) }}</textarea>
            </div>
